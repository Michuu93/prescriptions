...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<?> handleDataAccessResourceFailureException(WebRequest req, Exception ex) {
        logger.error(req.toString(), ex);
        ApiError apiError = ApiError.builder().httpStatus(HttpStatus.SERVICE_UNAVAILABLE).errorCode(ApiErrorCode.DATABASE_UNAVAILABLE).build();
        return buildResponseEntity(apiError);
    }
    ...
    protected ResponseEntity<?> handleNoSuchElementException(WebRequest req, Exception ex) {
        logger.error(req.toString(), ex);
        ApiError apiError = ApiError.builder().httpStatus(HttpStatus.NOT_FOUND).errorCode(ApiErrorCode.NO_SUCH_ELEMENT).build();
        return buildResponseEntity(apiError);
    }
    ...
    protected ResponseEntity<Object> handleHttpMediaTypeNotSupported(HttpMediaTypeNotSupportedException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        logger.error(request.toString(), ex);
        ApiError apiError = ApiError.builder().httpStatus(HttpStatus.UNSUPPORTED_MEDIA_TYPE).errorCode(ApiErrorCode.UNSUPPORTED_MEDIA_TYPE).build();
        return (ResponseEntity<Object>) buildResponseEntity(apiError);
    }
    ...
    protected ResponseEntity<Object> handleHttpRequestMethodNotSupported(HttpRequestMethodNotSupportedException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
        logger.error(request.toString(), ex);
        ApiError apiError = ApiError.builder().httpStatus(HttpStatus.METHOD_NOT_ALLOWED).errorCode(ApiErrorCode.METHOD_NOT_ALLOWED).build();
        return (ResponseEntity<Object>) buildResponseEntity(apiError);
    }
